    ...
    >
      <div className="w-full px-4 sm:px-6 lg:px-8">
        ...
        <div className="md:hidden w-full overflow-x-auto scrollbar-hide">
          <div className="flex space-x-4 py-2 px-2 min-w-max">
            {navItems.map((item) => (
              <motion.a
                key={item.name}
                href={item.href}
                className="text-gray-300 hover:text-white transition-colors whitespace-nowrap px-3 py-2 rounded-md bg-gray-800/50 flex-shrink-0"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                {item.name}
